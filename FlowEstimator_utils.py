...
def valMesh(x, y, mLayer):
    """Sample elevation value from mesh layer at point (x, y)"""
    try:
        point = QgsPointXY(x, y)
        # Get mesh renderer for data access
        mesh_renderer = mLayer.meshRenderer()
        if mesh_renderer is None:
            return None
        
        # Use the mesh's interpolation to get the value
        ...
        # Use the first dataset (typically the DEM/elevation)
        dataset_group_index = 0
        
        # Find a vertex or face dataset
        try:
            # Get the value at this point using interpolation
            from qgis.core import QgsMeshUtils
            # Try to interpolate the value at the given point
            value = mesh_renderer.interpolatedDatasetValue(point, dataset_group_index)
            return value
        except:
            # Fallback: try to get the closest vertex
            try:
                dataset_group = datasets[0][0]
                # Get mesh vertices and find closest one
                ...
                for vertex in vertices:
                    ...
                    if dist < min_dist:
                        ...
                        # This is simplified - actual implementation depends on dataset structure
                        closest_value = vertex.z() if hasattr(vertex, 'z') else None
